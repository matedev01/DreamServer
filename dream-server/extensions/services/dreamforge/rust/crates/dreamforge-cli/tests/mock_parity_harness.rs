// NOTE: This entire test file is disabled because the mock-anthropic-service
// crate has been removed. These tests need a replacement mock strategy before
...
// use mock_anthropic_service::{MockAnthropicService, SCENARIO_PREFIX};  // REMOVED: crate deleted
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("FORGE_CONFIG_HOME", &workspace.config_home)
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("forge should finish")
    } else {
...
fn prepare_auto_compact_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".forge").join("sessions");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "forge-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
