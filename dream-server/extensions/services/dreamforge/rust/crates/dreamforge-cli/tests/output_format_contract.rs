...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    assert_eq!(parsed["agents"][0]["source"]["id"], "project_forge");
    ...
    assert_eq!(parsed["agents"][2]["shadowed_by"]["id"], "project_forge");
}
...
fn assert_json_command_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Value {
    let output = run_forge(current_dir, args, envs);
    assert!(
...
fn run_forge(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
...
fn write_upstream_fixture(root: &Path) -> PathBuf {
    let upstream = root.join("dreamforge");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "forge-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
