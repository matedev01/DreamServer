...
use api::{oauth_token_is_expired, resolve_startup_auth_source, AnthropicClient, AuthSource};
use api::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Login { output_format } => {
            ...
            { eprintln!("login requires the `anthropic` feature"); std::process::exit(1); }
        }
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // DreamForge compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // DreamForge compat: --print makes output non-interactive
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-dreamforge:")
        .then_some(
            "Compatibility note: `/oh-my-dreamforge:*` is a DreamForge/OMC plugin command. DreamForge does not yet load plugin slash commands, statusline stdin, or OMC session hooks.",
        )
}
...
fn default_permission_mode() -> PermissionMode {
    env::var("DREAMFORGE_PERMISSION_MODE")
        .ok()
        .as_deref()
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        Ok(Some(token_set)) => {
            ...
            if expired {
                details.push(
                    "Suggested action  forge login to refresh local OAuth credentials".to_string(),
                );
            }
            DiagnosticCheck::new(
                "Auth",
...
fn dump_manifests(_output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    Err("dump-manifests is no longer available (compat-harness removed)".into())
}

fn print_bootstrap_plan(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let phases = runtime::BootstrapPlan::dreamforge_default()
        .phases()
        .iter()
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        scopes: vec![
            ...
            String::from("user:sessions:dreamforge"),
        ],
    }
}
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `forge` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Related          /status · forge --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            forge sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · forge status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            forge doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · forge --resume latest /doctor"
            .to_string(),
    }
}
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   forge --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(out, "  forge agents")?;
    writeln!(out, "  forge mcp show my-server")?;
    writeln!(out, "  forge /skills")?;
    writeln!(out, "  forge doctor")?;
    writeln!(out, "  forge login")?;
    writeln!(out, "  forge init")?;
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   forge --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("dreamforge-cli-{nanos}-{unique}"))
    }
    ...
    fn defaults_to_repl_when_no_args() {
        let _guard = env_lock();
        std::env::remove_var("DREAMFORGE_PERMISSION_MODE");
        assert_eq!(
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".forge")).expect("project config dir should exist");
        ...
        let original_permission_mode = std::env::var("DREAMFORGE_PERMISSION_MODE").ok();
        std::env::set_var("FORGE_CONFIG_HOME", &config_home);
        std::env::remove_var("DREAMFORGE_PERMISSION_MODE");
        ...
        match original_permission_mode {
            Some(value) => std::env::set_var("DREAMFORGE_PERMISSION_MODE", value),
            None => std::env::remove_var("DREAMFORGE_PERMISSION_MODE"),
        }
    ...
    fn env_permission_mode_overrides_project_config_default() {
        ...
        let original_permission_mode = std::env::var("DREAMFORGE_PERMISSION_MODE").ok();
        std::env::set_var("FORGE_CONFIG_HOME", &config_home);
        std::env::set_var("DREAMFORGE_PERMISSION_MODE", "read-only");
    ...
    fn formats_namespaced_omc_slash_command_with_contract_guidance() {
        let report = format_unknown_slash_command_message("oh-my-dreamforge:hud");
        assert!(report.contains("unknown slash command: /oh-my-dreamforge:hud"));
        assert!(report.contains("DreamForge/OMC plugin command"));
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("forge mcp"));
...
UU conflicted.rs",
    ...
    fn render_diff_report_shows_clean_tree_for_committed_repo() {
        ...
        git(&["config", "user.name", "DreamForge Tests"], &root);
    ...
    fn unknown_omc_slash_command_guidance_explains_runtime_gap() {
        let message = format_unknown_slash_command("oh-my-dreamforge:hud");
        assert!(message.contains("Unknown slash command: /oh-my-dreamforge:hud"));
        assert!(message.contains("DreamForge/OMC plugin command"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/dreamforge-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p dreamforge-cli"}"#)
                .contains("cargo test -p dreamforge-cli")
        );
        assert_eq!(
