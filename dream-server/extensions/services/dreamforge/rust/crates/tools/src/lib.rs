...
// ── DreamForge: web search result cache (last 10 queries) ──────────────────
fn search_cache() -> &'static Mutex<HashMap<String, Vec<SearchHit>>> {
    use std::sync::OnceLock;
    static CACHE: OnceLock<Mutex<HashMap<String, Vec<SearchHit>>>> = OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}

const SEARCH_CACHE_MAX: usize = 10;
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set DreamForge settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
/// Returns tool specs for DreamServer-native services.
/// Only includes tools whose backing service URL env var is configured.
pub fn dreamserver_tool_specs() -> Vec<ToolSpec> {
    let mut specs = Vec::new();

    if std::env::var("COMFYUI_URL").is_ok() {
        specs.push(ToolSpec {
            name: "GenerateImage",
            description: "Generate an image using ComfyUI on the DreamServer service mesh. Sends a text prompt and optional size/step parameters, polls until the workflow completes, and returns the path to the downloaded result image.",
            input_schema: json!({
                "type": "object",
                "properties": {
    ...
    if std::env::var("QDRANT_URL").is_ok() && std::env::var("EMBEDDINGS_URL").is_ok() {
        specs.push(ToolSpec {
            name: "RAGSearch",
            description: "Semantic search over indexed document collections via the DreamServer RAG pipeline. Embeds the query via TEI and searches Qdrant for the top-K most relevant text chunks.",
            input_schema: json!({
                "type": "object",
                "properties": {
    ...
    if std::env::var("DASHBOARD_API_URL").is_ok() {
        specs.push(ToolSpec {
            name: "ServiceHealth",
            description: "Query the DreamServer dashboard for service health, container status, and GPU utilisation. Can list all services, check a specific service, or retrieve detailed GPU stats.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "service": { "type": "string", "description": "Service name to filter (only for action=status)" },
                    "action": {
                        "type": "string",
                        "enum": ["status", "list", "gpu"],
                        "description": "What to query: list all services, status of one service, or GPU details"
                    }
                },
                "required": ["action"],
                "additionalProperties": false
    ...
    if std::env::var("TTS_URL").is_ok() {
        specs.push(ToolSpec {
            name: "TextToSpeech",
            description: "Convert text to speech audio via the Kokoro TTS service on DreamServer. Returns the path to the generated MP3 file in the workspace.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "text": { "type": "string", "description": "Text to synthesize into speech" },
                    "voice": { "type": "string", "description": "Voice identifier (default: \"af_heart\")" },
                    "output_path": { "type": "string", "description": "Output file path relative to workspace (default: auto-generated)" }
                },
                "required": ["text"],
                "additionalProperties": false
            }),
    ...
    if std::env::var("WHISPER_URL").is_ok() {
        specs.push(ToolSpec {
            name: "SpeechToText",
            description: "Transcribe an audio file to text via the Whisper service on DreamServer. Accepts common audio formats (wav, mp3, ogg, flac, m4a).",
            input_schema: json!({
                "type": "object",
                "properties": {
...
/// Returns all tool specs: MVP built-ins + DreamServer-native tools.
pub fn all_tool_specs() -> Vec<ToolSpec> {
    let mut specs = mvp_tool_specs();
    specs.extend(dreamserver_tool_specs());
    specs
}
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        // DreamServer-native tools
        "GenerateImage" => from_value::<GenerateImageInput>(input).and_then(run_generate_image),
        "RAGSearch" => from_value::<RAGSearchInput>(input).and_then(run_rag_search),
        "ServiceHealth" => from_value::<ServiceHealthInput>(input).and_then(run_service_health),
        "TextToSpeech" => from_value::<TextToSpeechInput>(input).and_then(run_text_to_speech),
        "SpeechToText" => from_value::<SpeechToTextInput>(input).and_then(run_speech_to_text),
...
fn run_bash(input: BashCommandInput) -> Result<String, String> {
    // ── DreamForge: block commands from DREAMFORGE_BLOCKED_COMMANDS env ──
    let command_trimmed = input.command.trim();
    let is_dreamserver_cross_exec = command_trimmed.starts_with("docker exec dream-");

    if !is_dreamserver_cross_exec {
        if let Ok(extra_blocked) = std::env::var("DREAMFORGE_BLOCKED_COMMANDS") {
            let blocked: Vec<&str> = extra_blocked.split(',').map(str::trim).collect();
            let first_word = command_trimmed.split_whitespace().next().unwrap_or("");
            for b in &blocked {
                if !b.is_empty()
                    && (first_word == *b || command_trimmed.starts_with(&format!("{b} ")))
                {
                    return Err(format!(
                        "Command blocked by DREAMFORGE_BLOCKED_COMMANDS: {b}"
                    ));
                }
            }
        }
    ...
    let mut output = execute_bash(input).map_err(|error| error.to_string())?;

    // ── DreamForge: append container context to successful output ────────
    if output.stderr.is_empty() || !output.stdout.is_empty() {
        let container_info = detect_container_context();
        if !container_info.is_empty() {
            if !output.stdout.is_empty() && !output.stdout.ends_with('\n') {
                output.stdout.push('\n');
            }
            output.stdout.push_str(&container_info);
        }
    }
...
/// Detect if running inside Docker and return a context line, or empty string.
fn detect_container_context() -> String {
    let in_docker = Path::new("/.dockerenv").exists();
    if !in_docker {
        return String::new();
    }
    let hostname = std::env::var("HOSTNAME").unwrap_or_else(|_| "unknown".to_string());
    format!("[DreamForge container context] running in Docker (host: {hostname})")
}
...
fn run_read_file(input: ReadFileInput) -> Result<String, String> {
    let mut output = read_file(&input.path, input.offset, input.limit).map_err(io_to_string)?;

    // ── DreamForge: append context info about the file ──────────────────
    let context_note = dreamforge_file_context(&output.file.file_path);
    if !context_note.is_empty() {
        output.file.content.push_str(&format!("\n\n{context_note}"));
    }

    to_pretty_json(output)
}

/// Return a brief DreamForge context note for a file path, or empty string.
fn dreamforge_file_context(path: &str) -> String {
    let normalized = path.replace('\\', "/");
    let mut notes = Vec::new();

    if normalized.starts_with("/workspace") || normalized.contains("/workspace/") {
        notes.push("workspace file");
    }
    if normalized.contains("node_modules") || normalized.contains("target/") {
        notes.push("dependency/build artifact");
    }

    if notes.is_empty() {
        String::new()
    } else {
        format!("[DreamForge context: {}]", notes.join(", "))
    }
}

#[allow(clippy::needless_pass_by_value)]
fn run_write_file(input: WriteFileInput) -> Result<String, String> {
    // ── DreamForge: smart filename inference when path is the default ────
    let path = if input.path == "output.txt" {
        infer_filename_from_content(&input.content)
    } else {
        input.path.clone()
    };

    let result = write_file(&path, &input.content).map_err(io_to_string)?;

    // ── DreamForge: make shebang files executable ───────────────────────
    if input.content.starts_with("#!") {
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            if let Ok(meta) = std::fs::metadata(&result.file_path) {
                let mut perms = meta.permissions();
                let mode = perms.mode() | 0o111; // add execute bits
                perms.set_mode(mode);
                let _ = std::fs::set_permissions(&result.file_path, perms);
            }
        }
    }
...
/// Infer a better filename from content when the user didn't provide a path.
fn infer_filename_from_content(content: &str) -> String {
    let trimmed = content.trim_start();
    if trimmed.starts_with("#!/usr/bin/env python") || trimmed.starts_with("import ") {
        return "output.py".to_string();
    }
    if trimmed.starts_with("#!/bin/bash") || trimmed.starts_with("#!/bin/sh") {
        return "output.sh".to_string();
    }
    if trimmed.starts_with('{') {
        // Quick JSON heuristic: starts with { and contains a quote
        if trimmed.contains('"') {
            return "output.json".to_string();
        }
    }
    if trimmed.starts_with("<!DOCTYPE") || trimmed.starts_with("<html") {
        return "output.html".to_string();
    }
...
struct WriteFileInput {
    /// File path — accepts `path` or `file_path`, defaults to "output.txt" if omitted.
    #[serde(default = "default_write_path", alias = "file_path", alias = "filename")]
    path: String,
    content: String,
}
...
// ── DreamServer-native tool input structs ────────────────────────────────────
...
fn execute_web_search(input: &WebSearchInput) -> Result<WebSearchOutput, String> {
    ...
    // ── DreamForge: check result cache first ────────────────────────────
    let cache_key = input.query.clone();
    if let Ok(cache) = search_cache().lock() {
        if let Some(cached_hits) = cache.get(&cache_key) {
            let mut hits = cached_hits.clone();
            apply_domain_filters(&mut hits, &input.allowed_domains, &input.blocked_domains);
            let summary = build_search_summary(&input.query, &hits, true);
    ...
    let client = build_http_client()?;
    let (search_url, format) = build_search_url(&input.query)?;
    let response = client
        .get(search_url)
        .send()
        .map_err(|error| error.to_string())?;
    ...
    // ── Parse results based on response format ──────────────────────────
    let mut hits = match format {
        SearchUrlFormat::Json => parse_searxng_json(&body),
        SearchUrlFormat::Html => {
            let mut h = extract_search_hits(&body);
            if h.is_empty() && final_url.host_str().is_some() {
                h = extract_search_hits_from_generic_links(&body);
            }
            h
        }
    };
    ...
    // ── DreamForge: store in cache (evict oldest if at capacity) ────────
    ...
    let summary = build_search_summary(&input.query, &hits, false);
...
/// Apply allowed/blocked domain filters to search hits.
...
/// Render a human-readable summary line for search results.
fn build_search_summary(query: &str, hits: &[SearchHit], cached: bool) -> String {
    let cache_note = if cached { " (cached)" } else { "" };
    if hits.is_empty() {
        format!("No web search results matched the query {:?}.{cache_note}", query)
    } else {
        let rendered_hits = hits
            ...
            .join("\n");
        format!(
            "Search results for {:?}{cache_note}. Include a Sources section in the final answer.\n{}",
            query, rendered_hits
        )
    }
}
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("dreamforge-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
// ── DreamServer-native tool run/execute functions ────────────────────────────

#[allow(clippy::needless_pass_by_value)]
fn run_generate_image(input: GenerateImageInput) -> Result<String, String> {
    to_pretty_json(execute_generate_image(input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_rag_search(input: RAGSearchInput) -> Result<String, String> {
    to_pretty_json(execute_rag_search(input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_service_health(input: ServiceHealthInput) -> Result<String, String> {
    to_pretty_json(execute_service_health(input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_text_to_speech(input: TextToSpeechInput) -> Result<String, String> {
    to_pretty_json(execute_text_to_speech(input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_speech_to_text(input: SpeechToTextInput) -> Result<String, String> {
    to_pretty_json(execute_speech_to_text(input)?)
}

fn execute_generate_image(input: GenerateImageInput) -> Result<Value, String> {
    let base_url = std::env::var("COMFYUI_URL")
        .unwrap_or_else(|_| "http://comfyui:8188".to_string());
    let client = build_http_client_with_timeout(120)?;

    let width = input.width.unwrap_or(1024);
    let height = input.height.unwrap_or(1024);
    let steps = input.steps.unwrap_or(20);

    // Build a minimal ComfyUI workflow payload
    let workflow = json!({
        "prompt": {
            "3": {
                "class_type": "KSampler",
            ...
            "9": {
                ...
                "inputs": { "filename_prefix": "dreamforge", "images": ["8", 0] }
            }
        }
    });
    ...
    let queue_resp = client
        ...
        .json(&workflow)
        .send()
        .map_err(|e| format!("ComfyUI queue request failed: {e}"))?;

    if !queue_resp.status().is_success() {
        let status = queue_resp.status();
        let body = queue_resp.text().unwrap_or_default();
        return Err(format!("ComfyUI returned {status}: {body}"));
    }

    let queue_json: Value = queue_resp.json().map_err(|e| e.to_string())?;
    let prompt_id = queue_json["prompt_id"]
        .as_str()
        .ok_or("ComfyUI response missing prompt_id")?
        .to_string();
    ...
    loop {
        ...
        let hist_resp = client
            .get(&history_url)
            .send()
            .map_err(|e| format!("ComfyUI history poll failed: {e}"))?;
        let hist: Value = hist_resp.json().map_err(|e| e.to_string())?;

        if let Some(entry) = hist.get(&prompt_id) {
            // Extract the output image filename from the SaveImage node
            if let Some(outputs) = entry.get("outputs") {
                for (_node_id, node_out) in outputs.as_object().into_iter().flatten() {
                    if let Some(images) = node_out.get("images").and_then(|v| v.as_array()) {
                        if let Some(first) = images.first() {
                            ...
                            // Download the image
                            let mut dl_url = format!("{base_url}/view?filename={filename}");
                            if !subfolder.is_empty() {
                                dl_url.push_str(&format!("&subfolder={subfolder}"));
                            }

                            let img_resp = client
                                .get(&dl_url)
                                .send()
                                .map_err(|e| format!("ComfyUI image download failed: {e}"))?;
                            ...
                            return Ok(json!({
                                ...
                                "steps": steps,
                                "prompt_id": prompt_id,
                                "duration_seconds": poll_start.elapsed().as_secs_f64()
                            }));
                        }
                    }
                }
            }
            return Err("ComfyUI completed but no output images found".to_string());
        }
    }
}
...
fn execute_rag_search(input: RAGSearchInput) -> Result<Value, String> {
    let qdrant_url = std::env::var("QDRANT_URL")
        .map_err(|_| "RAGSearch requires QDRANT_URL to be configured")?;
    let embeddings_url = std::env::var("EMBEDDINGS_URL")
        .map_err(|_| "RAGSearch requires EMBEDDINGS_URL to be configured")?;
    let client = build_http_client_with_timeout(15)?;

    let collection = input.collection.unwrap_or_else(|| "documents".to_string());
    let limit = input.limit.unwrap_or(5);

    // Step 1: Embed the query via TEI
    let embed_resp = client
        .post(format!("{embeddings_url}/embed"))
        .json(&json!({ "inputs": input.query }))
        .send()
        .map_err(|e| format!("Embedding request failed: {e}"))?;

    if !embed_resp.status().is_success() {
        let status = embed_resp.status();
        let body = embed_resp.text().unwrap_or_default();
        return Err(format!("Embedding service returned {status}: {body}"));
    }

    let embed_json: Value = embed_resp.json().map_err(|e| e.to_string())?;

    // TEI returns either a flat array or array-of-arrays; normalise to a single vector.
    let vector = if embed_json.is_array() {
        if embed_json[0].is_array() {
            embed_json[0].clone()
        } else {
            embed_json.clone()
        }
    } else {
        return Err("Unexpected embedding response format".to_string());
    };
    ...
    if !search_resp.status().is_success() {
        ...
        return Err(format!("Qdrant returned {status}: {body}"));
    }

    let search_json: Value = search_resp.json().map_err(|e| e.to_string())?;
    let results = search_json
        .get("result")
        .cloned()
        .unwrap_or_else(|| json!([]));

    // Extract text chunks with scores
    let chunks: Vec<Value> = results
        .as_array()
        .unwrap_or(&Vec::new())
        .iter()
        .map(|hit| {
            let score = hit.get("score").cloned().unwrap_or(json!(0.0));
            let payload = hit.get("payload").cloned().unwrap_or(json!({}));
            let text = payload
                .get("text")
                .or_else(|| payload.get("content"))
                .or_else(|| payload.get("chunk"))
                .cloned()
                .unwrap_or_else(|| json!("(no text field)"));
            let source = payload
                .get("source")
...
fn execute_service_health(input: ServiceHealthInput) -> Result<Value, String> {
    let dashboard_url = std::env::var("DASHBOARD_API_URL")
        .map_err(|_| "ServiceHealth requires DASHBOARD_API_URL to be configured")?;
    let client = build_http_client_with_timeout(5)?;

    match input.action.as_str() {
        "list" => {
            let resp = client
                .get(format!("{dashboard_url}/services"))
                .send()
                .map_err(|e| format!("Dashboard request failed: {e}"))?;
            if !resp.status().is_success() {
                let status = resp.status();
                let body = resp.text().unwrap_or_default();
                return Err(format!("Dashboard returned {status}: {body}"));
            }
            let body: Value = resp.json().map_err(|e| e.to_string())?;
            Ok(json!({ "action": "list", "services": body }))
        }
        "status" => {
            let resp = client
                .get(format!("{dashboard_url}/services"))
            ...
            if let Some(filter_name) = &input.service {
                let lower_filter = filter_name.to_lowercase();
                let empty = Vec::new();
                let all_services = body.as_array().unwrap_or(&empty);
                let filtered: Vec<&Value> = all_services
                    .iter()
                    .filter(|svc| {
                        svc.get("name")
                            .and_then(|n| n.as_str())
                            .map(|n| n.to_lowercase().contains(&lower_filter))
                            .unwrap_or(false)
                    })
                    .collect();
                Ok(json!({ "action": "status", "service": filter_name, "results": filtered }))
            } else {
                Ok(json!({ "action": "status", "services": body }))
            }
        }
        "gpu" => {
            let resp = client
                .get(format!("{dashboard_url}/api/gpu/detailed"))
                .send()
                .map_err(|e| format!("Dashboard GPU request failed: {e}"))?;
            if !resp.status().is_success() {
                let status = resp.status();
                let body = resp.text().unwrap_or_default();
                return Err(format!("Dashboard returned {status}: {body}"));
            ...
            let body: Value = resp.json().map_err(|e| e.to_string())?;
            Ok(json!({ "action": "gpu", "gpu_stats": body }))
        }
        other => Err(format!(
            "ServiceHealth: unknown action '{other}'. Use 'list', 'status', or 'gpu'."
        )),
    }
}

fn execute_text_to_speech(input: TextToSpeechInput) -> Result<Value, String> {
    let tts_url = std::env::var("TTS_URL")
        .map_err(|_| "TextToSpeech requires TTS_URL to be configured")?;
    let client = build_http_client_with_timeout(30)?;

    let voice = input.voice.unwrap_or_else(|| "af_heart".to_string());
    ...
    if !resp.status().is_success() {
        ...
        return Err(format!("TTS service returned {status}: {err_body}"));
    }
...
fn execute_speech_to_text(input: SpeechToTextInput) -> Result<Value, String> {
    let whisper_url = std::env::var("WHISPER_URL")
        .map_err(|_| "SpeechToText requires WHISPER_URL to be configured")?;
    let client = build_http_client_with_timeout(60)?;

    let audio_path = Path::new(&input.audio_path);
    if !audio_path.exists() {
        return Err(format!("Audio file not found: {}", input.audio_path));
    }
    ...
    let resp = client
        .post(format!("{whisper_url}/v1/audio/transcriptions"))
        .multipart(form)
        .send()
    ...
    if !resp.status().is_success() {
        let status = resp.status();
        let err_body = resp.text().unwrap_or_default();
        return Err(format!("Whisper service returned {status}: {err_body}"));
    }

    let resp_json: Value = resp.json::<Value>().map_err(|e| e.to_string())?;
    let text = resp_json
        .get("text")
...
/// Indicates whether the resolved search URL returns JSON (SearXNG) or HTML.
enum SearchUrlFormat {
    Html,
    Json,
}

/// Build a search URL, returning the format so callers know how to parse the
/// response.  Priority: `DREAMFORGE_WEB_SEARCH_BASE_URL` (backward compat,
/// HTML) > `SEARXNG_URL` (JSON) > DuckDuckGo HTML fallback.
fn build_search_url(query: &str) -> Result<(reqwest::Url, SearchUrlFormat), String> {
    // --- DreamForge engine: prefix support (e.g. "engine:github rest api") ---
    let (effective_query, engine) = extract_engine_prefix(query);

    // 1. Legacy env var — backward compat, always HTML
    if let Ok(base) = std::env::var("DREAMFORGE_WEB_SEARCH_BASE_URL") {
        let mut url = reqwest::Url::parse(&base).map_err(|e| e.to_string())?;
        url.query_pairs_mut().append_pair("q", effective_query);
        if let Some(eng) = &engine {
            url.query_pairs_mut().append_pair("engines", eng);
        }
        return Ok((url, SearchUrlFormat::Html));
    }

    // 2. SearXNG — JSON format
    if let Ok(searxng_base) = std::env::var("SEARXNG_URL") {
        let base = searxng_base.trim_end_matches('/');
        let mut url =
            reqwest::Url::parse(&format!("{base}/search")).map_err(|e| e.to_string())?;
        url.query_pairs_mut()
        ...
        return Ok((url, SearchUrlFormat::Json));
    }
    ...
    let mut url = reqwest::Url::parse("https://html.duckduckgo.com/html/")
        .map_err(|e| e.to_string())?;
    url.query_pairs_mut().append_pair("q", effective_query);
    Ok((url, SearchUrlFormat::Html))
}

/// If `query` starts with `engine:<name> `, extract the engine name and return
/// the remaining query.  Otherwise return the full query with no engine.
fn extract_engine_prefix(query: &str) -> (&str, Option<String>) {
    if let Some(rest) = query.strip_prefix("engine:") {
        if let Some(space_idx) = rest.find(' ') {
            let engine = &rest[..space_idx];
            let remaining = rest[space_idx..].trim_start();
            return (remaining, Some(engine.to_string()));
        }
    }
    (query, None)
}

/// Parse a SearXNG JSON response into `SearchHit`s.
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("DREAMFORGE_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".dreamforge-todos.json"))
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(forge_config_home) = std::env::var("FORGE_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&forge_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.forge/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".forge"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".forge"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("dreamforge-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("DREAMFORGE_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".dreamforge-agents"));
    }
    Ok(cwd.join(".dreamforge-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "forgeguide" | "forgeguideagent" | "guide" => String::from("forge-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".forge").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("FORGE_CONFIG_HOME") {
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("dreamforge-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::set_var(
            "DREAMFORGE_WEB_SEARCH_BASE_URL",
        ...
        std::env::remove_var("DREAMFORGE_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("DREAMFORGE_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("DREAMFORGE_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("DREAMFORGE_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".forge").join("skills").join("plan");
        let command_dir = root.join(".forge").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".forge/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".forge/commands/handoff.md"));
    ...
    fn skill_loads_project_local_claude_skill_prompt() {
        ...
        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("FORGE_CONFIG_HOME").ok();
        ...
        match original_config_home {
            Some(value) => std::env::set_var("FORGE_CONFIG_HOME", value),
            None => std::env::remove_var("FORGE_CONFIG_HOME"),
        }
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("DREAMFORGE_AGENT_STORE", &dir);
        ...
        std::env::remove_var("DREAMFORGE_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".forge")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".forge")).expect("cwd dir");
        std::fs::write(
            home.join(".forge").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".forge").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".forge").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        assert_eq!(output["task_packet"]["repo"], "parity");
        assert_eq!(
