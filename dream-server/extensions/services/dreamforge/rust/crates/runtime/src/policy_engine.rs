...
//! DreamForge configurable policy engine.
//!
//! The policy engine evaluates a prioritized set of [`PolicyRule`]s against
//! a [`LaneContext`] snapshot to determine which [`PolicyAction`]s should
//! fire.  Rules are sorted by ascending priority so that broad catch-all
//! rules run after specific high-priority ones.  Actions may be chained
//! and are flattened into a single ordered list before execution.
//!
//! DreamServer operators can define custom rule sets per-team or per-project,
//! enabling workflows like auto-merge-to-dev on green CI, stale-branch
//! cleanup, and startup-failure recovery without code changes.
