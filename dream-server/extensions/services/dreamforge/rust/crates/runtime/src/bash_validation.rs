...
///
/// DreamForge addition: `docker` and `kubectl` are included because DreamForge
/// runs inside containers and these commands can destroy the hosting environment.
const ALWAYS_DESTRUCTIVE_COMMANDS: &[&str] = &["shred", "wipefs", "docker", "kubectl"];
