...
//! DreamForge permission model.
//!
//! Permissions in DreamForge follow a layered evaluation strategy:
//!
//! 1. **Deny rules** are checked first and short-circuit all other logic.
//! 2. **Hook overrides** (`PermissionContext`) allow orchestration layers
//!    (e.g. lane policies or the DreamServer control plane) to force-allow,
//!    force-deny, or request interactive confirmation.
//! 3. **Ask rules** always trigger a prompt, even when the active mode would
//!    otherwise permit the operation.
//! 4. **Allow rules** and **mode comparison** handle the common case.
//!
//! The five permission modes form an ordered escalation ladder:
...
pub enum PermissionMode {
    ReadOnly,
    WorkspaceWrite,
    /// Grants unrestricted access to all tools including destructive operations.
    /// Also referred to as "DangerFullAccess" in DreamForge configuration files.
    DangerFullAccess,
    Prompt,
    Allow,
}
