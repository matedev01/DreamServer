...
//!
//! DreamForge extends the upstream usage model with local inference awareness:
//! when a request is served by a local model instead of the Anthropic API,
//! `local_inference_ms` on [`TokenUsage`] records wall-clock inference time
//! so operators can compare latency/cost trade-offs.
...
pub struct TokenUsage {
    ...
    /// Wall-clock milliseconds spent in local inference (DreamForge extension).
    /// `None` when the turn was served by the remote Anthropic API.
    pub local_inference_ms: Option<u64>,
}
